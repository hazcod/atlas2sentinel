...
type Config struct {
	...
	Atlas struct {
		PublicKey    string `yaml:"public_key" env:"ATLAS_PUB_KEY"`
		PrivateKey   string `yaml:"private_key" env:"ATLAS_PRIV_KEY"`
		LookBackDays uint   `yaml:"lookback_days" env:"ATLAS_LOOKBACK_DAYS"`
		URL          string `yaml:"url" env:"ATLAS_URL"`
	} `yaml:"atlas"`
...
func (c *Config) Validate() error {
	...
	if c.Atlas.PublicKey == "" {
		c.Atlas.LookBackDays = defaultLookback
	}

	if c.Atlas.PrivateKey == "" || c.Atlas.PublicKey == "" {
		return errors.New("empty atlas public or private key provided")
	}
